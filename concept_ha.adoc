...
Você pode identificar facilmente o endereço IP correto do Cloud Manager. A imagem a seguir mostra a exibição do sistema de armazenamento:
...
A imagem a seguir mostra a Exibição de volume:

image:screenshot_mount_volume_view.gif["Captura de tela: Mostra as opções de menu para um volume, que inclui a opção montar."]
